-- Production Admin Panel Database Schema (Complete Fix)
-- Handles all existing tables and missing columns
...
values 
  (gen_random_uuid(), 'John Smith', 'Tech Corp', true),
  (gen_random_uuid(), 'Sarah Johnson', 'Design Studio', true),
  (gen_random_uuid(), 'Mike Wilson', 'Consulting LLC', true),
  (gen_random_uuid(), 'Lisa Brown', 'Marketing Agency', false),
  (gen_random_uuid(), 'David Lee', 'Software Solutions', true);

-- Create sample sales entries
insert into public.sales_entries (user_id, customer_name, amount, product_service, sale_date)
select 
  (select id from public.user_profiles where is_active = true limit 1),
